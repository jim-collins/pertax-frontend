...
class PublicControllerSpec extends BaseSpec  {
  ...
  trait LocalSetup {
    lazy val controller = {
      ...
      when(c.configDecorator.feedbackSurveyThankYouUrl) thenReturn "/feedback-survey/thankYou?origin=PERTAX"
  ...
  "Calling PublicController.redirectToExitSurveyThankYou" should {

    "return 303" in new LocalSetup {

      val r = controller.redirectToExitSurveyThankYou()(buildFakeRequestWithAuth("GET"))
      status(r) shouldBe SEE_OTHER
      redirectLocation(r) shouldBe Some("/feedback-survey/thankYou?origin=PERTAX")
    }
  }
